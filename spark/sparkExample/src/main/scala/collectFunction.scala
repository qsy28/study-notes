...
/**
 * collect(): df to array
 * collect() action function is used to retrieve all elements
 * from the dataset (RDD/DataFrame/Dataset) as a Array[Row] to the driver program.
...
object collectFunction extends App{
  ...
  println(colList)
  // collect() does not return a Dataframe instead, it returns data in an array to your driver.
